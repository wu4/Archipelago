...
def create_regions_and_events(self: World):
 ...
 {%- for node_from, info in node_infos -%}
  {%- if info.items_every_room -%}
  {%- elif info.pickup -%}
   crl("{{node_from}}"),
  {%- elif info.event_name is not none -%}
   cre("{{node_from}}","{{info.event_name}}",{{not info.unskippable_event}}),
  {%- else -%}
   cr("{{node_from}}"),
  {%- endif -%}
 {%- endfor -%}))
 menu_region.add_exits({"Ship (Landing Site, Tallon Overworld)":"Start Game"})